  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (res?.error) {
      if (res.error.includes("Invalid password")) {
        ...
        toast.warning("Email or password missing");
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-md bg-white/80 backdrop-blur-md rounded-3xl shadow-xl p-8 border border-white/30">
        ...
        <div className="mt-6 text-xs text-gray-400">
          <p>By continuing, you agree to our <Link href="/terms-of-service" className="text-blue-500 hover:underline">Terms</Link> <span> & </span> <Link href="/privacy-policy" className="text-blue-500 hover:underline">Privacy Policy</Link>.</p>
        </div>
      </div>
    </div>
  );
